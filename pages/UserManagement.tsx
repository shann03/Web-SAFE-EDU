...
import { UserPlus, Search, Shield, Trash2, Mail, CheckCircle2, AlertCircle, X, Loader2, UserCog, ShieldAlert, ShieldCheck, RefreshCw } from 'lucide-react';
...
const UserManagement: React.FC<UserManagementProps> = ({ localUsers, onUpdateUser, onSync }) => {
      ...
      <div className="flex justify-between items-end border-b border-slate-200 pb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-100"
          >
                ...
                <tr key={user.id} className={`hover:bg-slate-50/50 transition-all group ${!user.is_active ? 'opacity-50 grayscale' : ''}`}>
                  ...
                  <td className="px-8 py-5">
                    <select 
                      ...
                      className="px-3 py-1.5 rounded text-[10px] font-black uppercase tracking-widest border bg-slate-50 border-slate-200 text-slate-700 outline-none"
                    >
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 bg-amber-50 border border-amber-100 rounded-lg text-amber-800 text-[10px] font-medium leading-relaxed">
                New accounts must be confirmed via secondary authentication before accessing student welfare data.
              </div>
              <input type="email" placeholder="Official Email Address" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none" />
              <button onClick={() => setIsModalOpen(false)} className="w-full bg-slate-900 text-white py-4 rounded-lg text-xs font-black uppercase tracking-widest">Grant Access Permission</button>
            </div>
          </div>
        </div>
      )}
